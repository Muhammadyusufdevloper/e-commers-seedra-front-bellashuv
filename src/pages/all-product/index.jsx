...
const AllProduct = () => {
  ...
  const products = productsData?.data?.products
    ...
    .map((product, index) => (
      <div key={index} className="all-products__card">
        <button
          onClick={() => dispatch(addWishlist(product))}
          className="all-products__card__img-like"
        >
          {wishlistData.some((el) => el.id === product.id) ? (
            <FaHeart color="crimson" />
          ) : (
        ...
        <div onClick={() => openModal(product)} className="all-products__card__img">
          <img src={product.urls[0] ? product.urls[0] : defaultImg} alt="" />
        </div>
        <div className="all-products__card__info">
          <div className="all-products__card__stars">
            {star.map((el, index) => (
              <div key={index} className="all-products__card__stars-icon">
                {el}
              </div>
            ))}
            <p>(123)</p>
          </div>
          <Link to={`single-rout/${product.id}`} className="all-products__card__text">
            {product.title}
          </Link>
          <div className="all-products__card__info__bottom">
            <h3 className="all-products__card__title-price">${product.price}</h3>
            <button
              onClick={() => {
                console.log("Adding to cart:", product);
                dispatch(add(product));
              }}
              className="all-products__card__title-icon"
            >
              <FiShoppingCart />
            </button>
          </div>
  ...
  const categories = categoryData?.data?.map((el) => (
    <li
      key={el.id}
      className={`all-products__category__item ${categoryFilter === el.id.toString() ? 'active' : ''}`}
  ...
  return (
    <div className="all-products container">
      <div className="all-products__top__info">
        <h2>All products.</h2>
        <button>View all (12)</button>
      </div>
      <ul className="all-products__category">
        <li
          className={`all-products__category__item ${!categoryFilter ? 'active' : ''}`}
      ...
      <div className="all-products__cards">{products}</div>
      {modal && (
